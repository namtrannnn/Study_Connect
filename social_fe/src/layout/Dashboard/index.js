...
function DashboardLayout({ children }) {
    ...
    const handleOpenSearch = () => setOpenPanel((prev) => (prev === 'search' ? null : 'search'));
    ...
    return (
        <div className="h-[100dvh] overflow-hidden bg-white text-gray-900 dark:bg-[#0f1117] dark:text-white">
                ...
                <div className="flex min-w-0 flex-1 overflow-hidden">
                    ...
                    >
                        <div className="mx-auto w-full max-w-[680px] px-4 py-4">
                            {children || <DashboardPage user={user} theme={theme} />}
                        </div>
                    </main>
            ...
            <button
                ...
                className={`fixed inset-0 z-30 hidden bg-black/10 backdrop-blur-[1px] transition-opacity duration-300 ease-out md:block ${isSearchOpen ? 'opacity-100' : 'pointer-events-none opacity-0'}`}
                aria-label="Đóng tìm kiếm"
            />

            {/* Search panel desktop */}
            <div
                className={`fixed bottom-4 top-4 z-40 hidden w-[420px] overflow-hidden rounded-[28px] border border-blue-100/80 bg-white/95 shadow-[0_24px_80px_rgba(15,23,42,0.22)] backdrop-blur-2xl transition-all duration-300 ease-out dark:border-white/10 dark:bg-[#181b22]/95 md:block ${isSearchOpen ? 'translate-x-0 scale-100 opacity-100' : '-translate-x-8 scale-[0.97] opacity-0 pointer-events-none'}`}
            ...
            {/* Search panel mobile */}
            <div
                className={`fixed inset-0 z-50 bg-white transition-all duration-300 ease-out dark:bg-[#181b22] md:hidden ${isSearchOpen ? 'translate-y-0 scale-100 opacity-100' : 'translate-y-6 scale-[0.98] opacity-0 pointer-events-none'}`}
            >
                <SearchPanel onClose={handleClosePanel} />
            </div>
        </div>
